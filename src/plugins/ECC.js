...
module.exports.decrypt = function (privateKey, data) {
	...
	// const ecdh1 = crypto.createECDH(curve).setPrivateKey(Buffer.from(privateKey, 'base64'));

	// const ecdh = crypto.
	ecdh.generateKeys();

	// const ecdh = ecKey.createECDH()
	const encrypted = typeof data === 'string' ? Buffer.from(data, 'base64') : data
	...
	const ciphertext = encrypted.slice(65, -16)
	...
	const cipher = crypto.createCipheriv(algorithm, aesKey, iv, {authTagLength: 16});
	// cipher.setAuthTag(tag);

	console.log('cipher',cipher)
	console.log('cipher1',cipher.update(ciphertext))
	console.log('cipher2',cipher.final());
