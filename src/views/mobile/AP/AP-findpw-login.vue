<template>
  <Wrap>
    <Header prev="prev" close="close" @click="onCloseClick"/>
    <Contents>
      <H3>비밀번호 재설정</H3>
      ...
      <Submit mt="24">
        <Button block large value="확인" @click="onBtnClick" :disabled="disabled"/>
      </Submit>
      ...
      <Alert :open="popupSucc">
        <template v-slot:dialog-title>
          <v-card-title>비밀번호가 재설정 되었습니다.</v-card-title>
        </template>
        <template v-slot:dialog-btn>
          <v-btn text block x-large @click="onBtnSuccess">확인</v-btn>
        </template>
      </Alert>
...
export default {
  ...
  methods: {
    ...
    onBtnClick() {
      ...
      var sendData = this.$route.params.txId+'||'+ sha256password

      console.log("sendData", sendData);
      this.$store.dispatch('auth/resetPassword', sendData).then(() => {
