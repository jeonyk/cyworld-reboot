<template>
  <Birthday>
    ...
    <ListBirthday class="overflow-y-auto" v-if="birth.keys.length">
      <ul v-for="key in birth.keys" :key="key">
        <li class="date">{{ key }}</li>
        <li v-for="obj in birth.list[key]" :key="obj.did">
          <span class="thumb"><img :src="obj.profileImgUrl" alt=""></span>
          <div>
            <p class="desc">{{ obj.relationName }}</p>
            <p class="subtitle">{{ obj.nickname }}</p>
          </div>
        </li>
      </ul>
    </ListBirthday>
    <InfiniteLoading  @infinite="infiniteFriendBirthDay">
