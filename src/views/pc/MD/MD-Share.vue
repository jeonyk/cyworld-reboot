<template>
  <Wrap>
        ...
        <v-card-text class="diary-share-list">
          <DiarySharePop>
            ...
            <DiaryShareList class="overflow">
              <p>멤버 (1/50)</p>
              <ListDiaryShareMember>
                <li>
                  <span class="thumb"><img src="/img/temp_profile.png" alt=""></span>
                  <div class="name_area">
                    <div>
                      <p class="desc"></p>
                      <p class="subtitle">나야나나야나(나)</p>
                    </div>
                  </div>
                  <div class="btn_area">
                    <Button value="공유끊기" width='86' outlined></Button>
                  </div>
                </li>
              </ListDiaryShareMember>
              <p>일촌목록</p>
              <ListDiaryShareMember>
                <li>
                  ...
                  <div class="name_area">
                    <div>
                      <p class="desc">지안, 편안함에 이르렀나?</p>
                      <p class="subtitle">이를 지, 편안할 안</p>
                    </div>
                  </div>
                  <div class="btn_area">
                    <Button value="초대하기" width='86' @click="diary_member_add"></Button>
                  </div>
                </li>
                <li>
                  <span class="thumb"><img src="/img/temp_profile.png" alt=""></span>
                  <div class="name_area">
                    <div>
                      <p class="desc">우린 늘 함께였지</p>
                      <p class="subtitle">김캐리</p>
                    </div>
                  </div>
                  <div class="btn_area">
                    <Button value="초대취소" width='86' outlined></Button>
                  </div>
                </li>
                <li>
                  <span class="thumb"><img src="/img/temp_profile.png" alt=""></span>
                  <div class="name_area">
                    <div>
                      <p class="desc">사이좋은 친구</p>
                      <p class="subtitle">신은혜</p>
                    </div>
                  </div>
                  <div class="btn_area">
                    <Button value="초대하기" width='86'></Button>
                  </div>
                </li>
                <li>
                  <span class="thumb"><img src="/img/temp_profile.png" alt=""></span>
                  <div class="name_area">
                    <div>
                      <p class="desc">영혼의 단짝</p>
                      <p class="subtitle">이조은</p>
                    </div>
                  </div>
                  <div class="btn_area">
                    <Button value="초대하기" width='86'></Button>
    ...
    <!-- 다이어리 멤버 초대 알림 -->
    <Alert cardClass="web" :open="pop_diary_member_add" max-width="300">
      <template v-slot:dialog-title>
        <v-card-title>이배추도사 님을 초대할까요?</v-card-title>
      </template>
      <template v-slot:dialog-btn>
        <v-btn class="two-btns cancel" text  x-large @click="pop_diary_member_add = false">취소</v-btn>
        <v-btn class="two-btns" text x-large @click="pop_diary_member_add_confirm = true;pop_diary_member_add = false;">확인</v-btn>
      </template>
    </Alert>
    <!-- 다이어리 멤버 초대 확인 -->
    <Alert cardClass="title-text web" :open="pop_diary_member_add_confirm" max-width="300">
      <template v-slot:dialog-title>
        <v-card-title>이배추도사 님을 초대했어요</v-card-title>
      </template>
      <template v-slot:dialog-text>
        <v-card-text>
          상대방이 초대를 수락하면 다이어리를 <br/>
          함께 작성할 수 있습니다.
        </v-card-text>
      </template>
      <template v-slot:dialog-btn>
        <v-btn text x-large block @click="pop_diary_member_add_confirm = false">확인</v-btn>
      </template>
    </Alert>
...
export default {
  ...
  data(){
    return {
      ...
      pop_shareList : false,
      pop_diary_member_add:false,
      pop_diary_member_add_confirm: false,
    }
  },
  mounted(){
  ...
  methods:{
    ...
    diary_member_add(){
      this.pop_diary_member_add = true;
    }
